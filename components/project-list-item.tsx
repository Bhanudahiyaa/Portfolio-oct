...
export function ProjectListItem({ title, bullets, tags, links = [], className }: ProjectListItemProps) {
  return (
    <article
      className={cn(
        ...
        <div className="ml-auto flex gap-2">
          {links.map((l) => (
            <a
              key={l.label}
              href={l.href}
              className="text-xs rounded-full border border-neutral-200 px-2 py-0.5 text-neutral-700 hover:bg-neutral-50 dark:border-neutral-800 dark:text-neutral-300 dark:hover:bg-neutral-900"
            >
              {l.label}
            </a>
          ))}
